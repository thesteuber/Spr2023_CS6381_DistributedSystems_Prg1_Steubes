...
class SubscriberMW ():
  ...
  def handle_reply (self, socket):
    ...
    try:
      ...
      if (disc_resp.msg_type == discovery_pb2.TYPE_REGISTER):
        ...
        self.logger.error ("SubscriberMW::handle_reply TYPE_ISREADY is deprecated!")
  ...
  def send_to_discovery_services(self, disc_req):
    ...
    if (self.lookup == "Chord"):
      ...
      self.logger.info ("SubscriberMW::send_to_ip_port successor connected to {}".format(connect_str))
      ...
      while True:
        try:
            ...
            if self.req in socks and socks[self.req] == zmq.POLLIN:
                self.logger.debug ("SubscriberMW::send_to_discovery_services - response received from Discovery service")
                return True
            
            # Timeout waiting for response, close and re-open REQ socket
            self.logger.debug ("SubscriberMW::send_to_discovery_services - timed out waiting for response from Discovery service, re-sending registration request")
            self.req.close()
            self.req = None
            self.poller.unregister(self.req)
            
        except Exception as e:
            self.logger.error("Exception caught in SubscriberMW::send_to_discovery_services: {}".format(e))
            return False
  ...
  def unregister (self, name, topiclist):
    ''' unregister the appln with the discovery service '''

    try:
      self.logger.info ("SubscriberMW::unregister")
      ...
      # Build the Registrant Info message first.
      self.logger.debug ("SubscriberMW::unregister - populate the Registrant Info")
      reg_info = discovery_pb2.RegistrantInfo () # allocate
      reg_info.id = name  # our id
      ...
      self.logger.debug ("SubscriberMW::unregister - done populating the Registrant Info")
      
      # Next build a RegisterReq message
      self.logger.debug ("SubscriberMW::unregister - populate the nested register req")
      register_req = discovery_pb2.UnregisterReq ()  # allocate 
      register_req.role = discovery_pb2.ROLE_SUBSCRIBER  # we are a publisher/subscriber as the broker
      # It was observed that we cannot directly assign the nested field here.
      # A way around is to use the CopyFrom method as shown
      register_req.info.CopyFrom (reg_info)  # copy contents of inner structure
      register_req.topiclist[:] = topiclist   # this is how repeated entries are added (or use append() or extend ()
      self.logger.debug ("SubscriberMW::unregister - done populating nested RegisterReq")

      # Finally, build the outer layer DiscoveryReq Message
      self.logger.debug ("SubscriberMW::unregister - build the outer DiscoveryReq message")
      disc_req = discovery_pb2.DiscoveryReq ()  # allocate
      disc_req.msg_type = discovery_pb2.TYPE_UNREGISTER  # set message type
      # It was observed that we cannot directly assign the nested field here.
      # A way around is to use the CopyFrom method as shown
      disc_req.unregister_req.CopyFrom (register_req)
      self.logger.debug ("SubscriberMW::unregister - done building the outer message")
